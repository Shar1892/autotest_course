...
class Segment:
    ...
    def length(self):
        length = ((self.end1[0] - self.end2[0]) ** 2 + (self.end1[1] - self.end2[1]) ** 2) ** 0.5
        return round(length, 2)

    def x_axis_intersection(self):
        return self.end1[1] <= 0 <= self.end2[1] or self.end2[1] <= 0 <= self.end1[1]

    def y_axis_intersection(self):
        return self.end1[0] <= 0 <= self.end2[0] or self.end2[0] <= 0 <= self.end1[0]
